...
class Face_Recognizer:
    ...
    def attendance(self, name):
        ...
        insert_query = f"INSERT INTO attendance (name, time, date) VALUES ('{name}', '{current_time}', '{current_date}')"
                ...
                if (self.current_frame_face_cnt == self.last_frame_face_cnt) and (
                    ...
                    for i in range(self.current_frame_face_cnt):
                        if self.current_frame_face_name_list[i] != "unknown":
                            text = str(self.current_frame_face_name_list[i])
                            img_rd = cv2.putText(img_rd, text, self.current_frame_face_position_list[i], self.font, 0.8, (0, 255, 255), 1,
                             cv2.LINE_AA)
